"""Visualize best and worst tiles by IoU."""
...
from wildfire.vis.tile_picker import pick_best_worst_tiles
...
def main() -> None:
    ...
    best, worst = pick_best_worst_tiles(
        val_pat,
        stats,
        model,
        int(k_patch),
    ...
    out_dir = Path(args.artifacts_dir) / "reports" / "plots"
    plot_tile_predictions(best, model, int(k_patch), thr_prob, base_cfg["train"]["batch_patches"], out_dir / "best_tile.png")
    plot_tile_predictions(worst, model, int(k_patch), thr_prob, base_cfg["train"]["batch_patches"], out_dir / "worst_tile.png")
    LOGGER.info("Saved best/worst plots to %s", out_dir)
